 ...
 */
public abstract class BaseRecord implements Serializable, Remote, IBaseRecord {
    protected static final Logger logger = LoggerFactory.getLogger(BaseRecord.class);
    ...
    private static final long serialVersionUID = -7482609339026723563L;
     ...
     */
    public BaseRecord(final byte[] newData, final String name) {
        setData(newData);
        setFilename(name);
        setCreationTimestamp(Instant.now());
    }
     ...
     */
    public BaseRecord(final byte[] newData, final String name, @Nullable final String form) {
        this(newData, name);
        if (form != null) {
    ...
    public BaseRecord(final byte[] newData, final String name, final String form, @Nullable final String fileType) {
     ...
     * 
     * <p>
     * Data returned from a backing Channel will be truncated at {@link BaseRecord#MAX_BYTE_ARRAY_SIZE}. Using
     ...
     * 
     * @return the length of theData, or the size of the seekable byte channel up to {@link BaseRecord#MAX_BYTE_ARRAY_SIZE}.
     */
    @Override
    @SuppressWarnings("UnnecessaryDefaultInEnumSwitch")
